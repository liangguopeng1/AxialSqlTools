    ...
    namespace IntelliSense
    {
        public partial class CompletionEngine
        {
            ...
            private static bool ObjectNameMatchesFilter(string objectName, string filter)
            {
                ...
                string collapsedName = CompletionMatcher.CollapseForMatch(objectName);
                string collapsedFilter = CompletionMatcher.CollapseForMatch(filter);
                if (!string.IsNullOrEmpty(collapsedFilter)
                    && collapsedName.IndexOf(collapsedFilter, StringComparison.OrdinalIgnoreCase) >= 0)
                    return true;
                int[] unused;
                return CompletionMatcher.TryMatchSubsequence(objectName, filter, out unused);
            }
            ...
            private List<CompletionItem> FilterAndSort(List<CompletionItem> items, string prefix, IntelliSenseSettings settings, ScriptFactoryAccess.ConnectionInfo connInfo, FromObjectNameContext fromName)
            {
                ...
                {
                    int[] indices = null;
                    if (string.IsNullOrEmpty(filterPrefix) || CompletionMatcher.GetMatchScore(item, filterPrefix, out indices) > 0)
                    {
                ...
                filtered.Sort((a, b) =>
                {
                    int scoreA = CompletionMatcher.GetMatchScore(a, filterPrefix);
                    int scoreB = CompletionMatcher.GetMatchScore(b, filterPrefix);
                ...
                if (filtered.Count > settings.maxCompletionItems)
                {
                    ...
                    if (!string.IsNullOrEmpty(filterPrefix))
                    {
                        ...
                        {
                            int boundaryScore = CompletionMatcher.GetMatchScore(filtered[keep - 1], filterPrefix);
                            while (keep < filtered.Count && keep < cap
                                && CompletionMatcher.GetMatchScore(filtered[keep], filterPrefix) == boundaryScore)
                                keep++;
                        }
                    }
            ...
            private static bool ItemMatchesPrefix(CompletionItem item, string filterPrefix)
            {
                ...
                return CompletionMatcher.GetMatchScore(item, filterPrefix) > 0;
            }
            ...
            private static bool ItemMatchesPrefix(string displayText, string filterPrefix)
            {
                if (string.IsNullOrEmpty(filterPrefix)) return true;
                if (string.IsNullOrEmpty(displayText)) return false;
                return CompletionMatcher.GetMatchScore(displayText, filterPrefix) > 0;
            }
            ...
            private static string GetLastSegment(string prefix) => CompletionMatcher.GetLastSegment(prefix);
